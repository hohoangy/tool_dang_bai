...
const bootTimeoutMs = Math.max(30_000, Number(options.bootTimeoutMs || 120_000));
    ...
    for (let attempt = 0; attempt <= retries; attempt += 1) {
      ...
      try {
        const result = await withTimeout(
          runReview(account, scenario, index),
          caseTimeoutMs,
          `${instance.label}/${scenario.id} vượt quá ${Math.round(caseTimeoutMs / 1000)} giây`
        );
...
async function ensureRealReviewDeviceReady(account, instance, scenario) {
  ...
  if (open?.target && open.target !== target) {
    account.deviceId = open.target;
  }
  const readyAfterOpen = await getAdbState(account.deviceId || target);
  ...
  while (Date.now() - startedAt < bootTimeoutMs) {
    await delay(5_000);
    last = await getAdbState(account.deviceId || target);
    if (last.ready) {
      const result = {
        ok: true,
        target: account.deviceId || target,
...
function isRetryableReviewError(message = '') {
  return /adb|device|system ui|không phản hồi|khong phan hoi|isn't responding|input tap|uiautomator|unknown|offline|share intent|chưa mở thành công|chua mo thanh cong|instagram|media|composer/i.test(String(message));
}
...
function actionForFailureReason(reason = '') {
  if (reason.startsWith('preflight_boot_failed:')) return 'Đây là lỗi preflight LDPlayer/ADB, chưa vào workflow Instagram. Tool đã thử restart ADB, thử attach localhost và đóng LD; mở LD thủ công đến khi adb devices thấy device rồi chạy lại, hoặc kiểm tra ADB Debugging trong LDPlayer.';
